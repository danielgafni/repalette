...
def pretrain(version, num_workers=7, max_epochs=None, sleep=1):
    if max_epochs:
        max_epochs_part = f"--version {version} --max-epochs {max_epochs} --num-workers {num_workers}"
    else:
        max_epochs_part = ""

    command = f"""
    python scripts/pretrain.py {max_epochs_part}
    sleep {sleep}
    """
    return command
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-workers", type=int, default=7, help="Number of workers for the dataloaders")
    parser.add_argument("--max-attempts", type=int, default=20, help="Number of attempts to run the tasks")
    ...
    workflow.add_task(
        func=pretrain,
        params=dict(version=task_name, max_epochs=1, sleep=args.sleep, num_workers=args.num_workers),
        uid=task_name,
        time_req=None,
